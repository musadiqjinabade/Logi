...
class ScanAttribute extends Component {
    ...
    //     this.setState({ model: true, props: this.props.navigation.state.params.item, deviceModel: model, jobId: this.props.navigation.state.params.item != 'jobHistory' ? this.props.navigation.state.params.item : this.props.navigation.state.params.id }, () => {
    //         this.assignment_mapping()
    //     });
    ...
    render() {
        ...
        return (
            <Container style={{ flex: 1, backgroundColor: '#F1EFEE' }}>
                <Headers isHome={true} onBackHome={() => { this.props.navigation.navigate('Dashboard') }} label={'Attributes - 129'} onBack={() => { this.props.navigation.goBack() }} expanded={this.state.expanded} />
    ...
    checkLevel(item, index) {
        var responseData = _.map(this.state.Updatedata, (data) => {
            if (data.assignment_mapping_id == item.assignment_mapping_id) {
                return {
                    ...data,
                    mode: data.mode === "Collapsed" ? 'Expanded' : 'Collapsed'
                }
            }
            else {
                return data
        ...
        this.setState({
            ...
            if (item.mode = 'Expanded') {
                ...
                if (array) {
                    this.setState({ Updatedata: array })
                }
                if (check) {
    ...
    async expandReceivedata(seqnum, index) {
        var url = APIService.URLBACKEND + 'assignment/viewjobhistory?job_id=' + this.state.jobId + '&sequence_number=' + seqnum + '&job_type=Assignment_Aggregation';
        var scan = await APIService.execute('GET', url, null)
        var i;
        if (scan.data.status_code == 200 && scan.data.data != null) {
                    ...
                    <View style={{ flex: 1, flexDirection: 'column', paddingLeft: wp('2%'), paddingRight: wp('2%'), marginBottom: hp('10%') }}>
                        <View style={{ width: wp('100%'), flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', alignSelf: 'center' }}>
                            ...
                            <TouchableOpacity style={{
                                ...
                                margin: wp('2%'),
                                elevation: 4,
                                backgroundColor: color.gradientEndColor, justifyContent: 'center', alignItems: 'center',
                            }}
                        ...
                        <TouchableOpacity style={styles.selectBackgroundStyle} onPress={() => { }}>
                            <Text style={styles.selectSubLocation}>{"Select Sub Location"}</Text>
                        ...
                        <Text style={styles.scanItems}>SKU 2020202020 Parle G</Text>
                        <Text style={styles.scanItems}>SKU 2020202021 Parle G</Text>
                        <Text style={styles.scanItems}>SKU 2020202022 Parle G</Text>
                        <Text style={styles.scanItems}>SKU 2020202023 Parle G</Text>
                        <Text style={styles.scanItems}>SKU 2020202024 Parle G</Text>
                        <Text style={styles.scanItems}>SKU 2020202025 Parle G</Text>
                        <Text style={styles.scanItems}>SKU 2020202026 Parle G</Text>
                        <Text style={styles.scanItems}>SKU 2020202027 Parle G</Text>
                    </View>
                </ScrollView>
                <View style={{ flex: 1 }}>
                    <View style={{
                        ...
                        <TouchableOpacity onPress={() => { this.props.navigation.navigate('AttributeScanningItemsScreen') }}>
                            <LinearGradient
...
export default withNavigation(ScanAttribute);
