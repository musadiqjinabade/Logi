...
class ScanSummary extends Component {
    ...
    async ScanSummary() {
        var scan = await APIService.execute('GET', APIService.URLBACKEND + 'mapping/getmappingsummary?mapping_id=' + this.state.mapping_id, null)
    ...
    async componentDidMount() {
        ...
        this.focusListener = this.props.navigation.addListener('didFocus', () => {
            ...
            this.setState({ mapping_id: this.props.navigation.state.params.item, loading: true }, async() => {
    ...
    async conformation() {
        ...
        this.refs.modal6.close()
        var scan = await APIService.execute('POST', APIService.URLBACKEND + 'mapping/endmappingjob', data)
    ...
    async expandReceivedata(seqnum, index) {
        var url = APIService.URLBACKEND + 'assignment/viewjobsummary?job_id=' + this.state.mapping_id + '&sequence_number=' + seqnum + '&job_type=Aggregation';
    ...
    async cancelJob() {
        ...
        var data = {}
        data.mapping_id = this.state.job_detail?this.state.job_detail.jobId:null
    ...
    render() {
        ...
        return (
            <Container style={{ flex: 1, backgroundColor: '#F1F3FD' }}>
                <Headers isHome={true} onBackHome={() => { this.refs.redirectHome.open() }}  label={this.state.job_detail ? 'Aggregate - '+this.state.job_detail.job_id : null} onBack={() => { this.props.navigation.navigate('LinkScanproduct') }} expanded={this.state.expanded} />
                    ...
                    <ScrollView style={{ flex: 1 }}>
                        <View style={{ flex: 1, flexDirection: 'column',  }}>
                            <View style={{  paddingLeft: wp('2%'), paddingRight: wp('2%'), justifyContent: 'space-between', flexDirection: 'row', marginTop: hp('1%')  }}>
                                <Button style={styles.buttonStyle} onPress={() => { this.props.navigation.navigate('LinkScanDetailScreen',{header:this.state.job_detail ? this.state.job_detail.job_id : 'Aggregate'}) }}>
