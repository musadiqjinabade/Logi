...
const selected_data = {}
class SearchableTags extends Component {
    ...
    async getData(){
        var data = await APIService.execute('GET', APIService.URLBACKEND + APIService.getAssignTags, null)
        ...
        this.setState({ searchtags: text }, () => {
            ...
            if (text != '') {
                ...
                for (var d of this.arrayholder) {
                    if ( d.tag_name.toLowerCase().includes(text.toLowerCase()) ) {
    ...
    changecheck(item) {
        this.setState({
            check_id: { ...this.state.check_id, [item.tag_id]: !this.state.check_id[item.tag_id]  }
        },()=>{
    ...
    async save(item) {
        ...
        for(let i=0; i<this.state.Tags.length; i++){
            for(let j=0; j<check_data.length; j++){
                if(check_data[j]==this.state.Tags[i].tag_id){
                    //push data
                    tag_data.push(this.state.Tags[i].tag_name)

                }
            }
        }
        await AsyncStorage.removeItem('Tagsdata');
        AsyncStorage.setItem('Tagsdata',JSON.stringify(check_data))
        await AsyncStorage.removeItem('SelectedTags');
        AsyncStorage.setItem('SelectedTags',JSON.stringify(tag_data))
        ...
        if (this.state.loading) {
            return (
                <SafeAreaView style={{flex:1}}>
                   <Headers  label={"Select Tags"} onBack={() => { this.props.navigation.goBack()  }} expanded={this.state.expanded} />
                                    ...
                                    renderItem={({ item }) => {
                                        ...
                                        return (
                                            <TouchableOpacity style={{ flexDirection: 'row', margin: wp('2%') }}
                                                ...
                                                >
                                                <CheckBox checked={this.state.check_id[item.tag_id]} style={{ borderRadius: 5, marginRight: 5, backgroundColor: this.state.check_id[item.tag_id] ? color.gradientStartColor : '#ffffff', borderColor: this.state.check_id[item.tag_id] ? color.gradientStartColor : color.gradientStartColor, marginTop: hp('1%'), width: 20, height: 20, justifyContent: 'center', alignSelf: 'center', alignItems: 'center', alignContent: 'center', paddingTop:hp('0.7%') }}
                                                    onPress={() => this.changecheck(item)}
                                                />
                                                <Text style={{ justifyContent: 'center', alignItems: 'center', marginHorizontal: wp('3%'), marginTop: wp('2%'), marginLeft: wp('6%'), fontFamily: 'Catamaran-Bold', color:this.state.check_id[item.tag_id] ? color.gradientStartColor:null }}>{item.tag_name}</Text>
                                            </TouchableOpacity>
                                        )
...
export default withNavigation(SearchableTags);
