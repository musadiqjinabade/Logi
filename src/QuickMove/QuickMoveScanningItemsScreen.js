...
class QuickMoveScanningItemsScreen extends Component {
    ...
    async ScanSummary() {
        var scan = await APIService.execute('GET', APIService.URLBACKEND + APIService.getQuickMoveSummary + '?quickmove_id=' + this.props.navigation.state.params.item, null)
        if (scan.data.status_code !== 400) {
            ...
            this.setState({ packagingDetails: scan.data.data.level_wise_count, productDetails: scan.data.data.product_wise_count, job_detail: scan.data.data.stockdetails[0], assignment_mapping: responseData, loading: false }, () => {
    ...
    async navigateBack() {
        this.props.navigation.navigate('QuickMoveScanningScreen')
        return true;
    }
    ...
    async conformation() {
        ...
        body = {}
        body.quickmove_id = this.props.navigation.state.params.item
        body.comment = this.state.comment
        body.location_id = this.state.locationId


        this.refs.modal6.close()
        var scan = await APIService.execute('POST', APIService.URLBACKEND + APIService.endQuickMoveJob, body)
    ...
    async expandReceivedata(seqnum, index) {
        var url = APIService.URLBACKEND + APIService.getExpandedData + '?job_id=' + this.props.navigation.state.params.item + '&sequence_number=' + seqnum + '&job_type=Quickmove';
    ...
    render() {
        ...
        return (
            <Container style={{ flex: 1, backgroundColor: '#F1F3FD' }}>
                <Headers onBackHome={() => { this.refs.redirectHome.open() }} isHome={true} label={'Quick Move - ' + this.props.navigation.state.params.item} onBack={() => { this.props.navigation.navigate('QuickMoveScanningScreen') }} expanded={this.state.expanded} />
                    ...
                    <ScrollView style={{ flex: 1 }}>
                        <View style={{ flex: 1, flexDirection: 'column' }}>
                            <View style={{ paddingLeft: wp('2%'), paddingRight: wp('2%'), justifyContent: 'space-between', flexDirection: 'row', marginTop: hp('1%') }}>
                                <Button style={styles.buttonStyle} onPress={() => { this.props.navigation.navigate('QuickMoveScanningDetailScreen', { item: this.props.navigation.state.params.item }) }}>
                                ...
                                <Button style={styles.buttonStyle}
                                    onPress={() => this.state.assignment_mapping.length === 0 ? alert("There is no item to delete.") : this.props.navigation.navigate('Deletebyscan', { item: seq, jobId: this.props.navigation.state.params.item, jobType: "Quickmove", jobhistroy: this.props.navigation.state.params.jobhistroy })}>
                            ...
                            <View style={{
                                flex: 1, marginVertical: hp('3%'), paddingLeft: wp('2%'), paddingRight: wp('2%')
                            }}>
                                    ...
                                    <FlatList
                                        ...
                                        renderItem={({ item, index }) => (
                                            <View style={{
                                                ...
                                                margin: wp('1%'), marginTop: wp('0.5%'), marginBottom: wp('0.5%'), justifyContent: 'flex-start', alignItems: 'center', marginVertical: hp('0.5%'), marginLeft: item.child_count === 0 && item.mode === "Collapsed" ? null : item.level_id == 1 ? wp('8%') : item.level_id == 2 ? item.child_count >= 0 ? null : wp('6%') : item.level_id == 3 ? item.child_count >= 0 ? null : wp('4%') : null
                                            }}>
                                                {
                                                    item.child_count !== 0 ?
...
export default withNavigation(QuickMoveScanningItemsScreen);
