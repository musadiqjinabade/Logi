...
export default class RMDropdown extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
    ...
    async componentDidMount() {
        this.setState({ loading: false }, async () => {
            var response = await APIService.execute('GET', APIService.URLBACKEND + 'rowmaterial/getrowmaterial?page_no=0&page_count=10&sort_on=created_at&sort_by=desc', null)
            ...
            if (response.data.status == 200) {
                if (response.data.data.length > 0) {
                    ...
                    ToastAndroid.show('No Raw material found. Please contact the administrator', ToastAndroid.LONG, 25, 50);
                    this.props.onSelect('');
                }
    ...
    async loadMoreJobs() {
        this.setState({ page_count: this.state.page_count + 1, loading: true }, async () => {
            var response = await APIService.execute('GET', APIService.URLBACKEND + 'rowmaterial/getrowmaterial?page_no=' + this.state.page_count + '&page_count=10&sort_on=created_at&sort_by=desc', null)
            console.log("response:", response)
            if (response.data.status == 200) {
        ...
        this.setState({ searchInput: text }, () => {
            if (text !== '') {
                ...
                for (var d of this.arrayholder) {
                    if (d.rm_name ? d.rm_name.toLowerCase().includes(text.toLowerCase()) : d.rm_name.toLowerCase().includes(text.toLowerCase())) {
    ...
    render() {
        console.log('arrray:', this.arrayholder)
        if (this.state.data) {
            return (
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) => (
                                    <ListItem onPress={() => this.selectItem(item)}>
                                        <Body>
                                            <Text style={{ color: "#263C88", fontFamily: 'Montserrat-Regular', }}>{item.rm_name || '---'}</Text>
                                        </Body>
                                    </ListItem>
                                )}
                                keyExtractor={item => item.rm_id}
                            />
