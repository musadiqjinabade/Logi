import {
    Text, View, StatusBar, StyleSheet, AsyncStorage, TouchableOpacity, ToastAndroid, Dimensions, ActivityIndicator,
    TextInput,
    BackHandler,
    FlatList,
    Platform
...
class AssignSummary extends Component {
    ...
    async ScanSummary() {
        var scan = await APIService.execute('GET', APIService.URLBACKEND + 'assignment/getjobsummary?job_id=' + this.state.mapping_id, null)
        ...
        this.setState({ job_detail: scan.data.data.job_detail[0], assignment_mapping: responseData, packaging_details: scan.data.data.packaging_details, product_details: scan.data.data.product_details, loading: false }, () => {
            if (this.state.assignment_mapping[0] != undefined && this.state.assignment_mapping[0].level_id == 2) {
    ...
    async conformation() {
        ...
        var data = {}
        data.job_id = this.state.mapping_id;
        ...
        this.setState({ message: scan.data.message }, () => {
            if (scan.data.status_code == 200) {
                AsyncStorage.setItem('comment','');
                AsyncStorage.setItem('assignjobId', this.state.mapping_id);
        ...
        this.setState({
            ...
            if (array) {
                if (item.mode == 'Collapsed') {
                    ...
                    this.setState({ assignment_mapping: response })
                }
                else {
                    this.setState({
    ...
    async expandReceivedata(seqnum, index) {
        var url = APIService.URLBACKEND + 'assignment/viewjobsummary?job_id=' + this.state.mapping_id + '&sequence_number=' + seqnum + '&job_type=Assignment_Aggregation';
    ...
    async cancelJob() {
        ...
        var data = {}
        data.mapping_id = this.state.mapping_id || null
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1, flexDirection: 'column', backgroundColor: '#F1F3FD' }}>
                <Headers isHome={true} onBackHome={() => { this.refs.redirectHome.open() }} label={'Scan Each Item - ' + this.state.mapping_id} onBack={() => { this.props.navigation.goBack() }} expanded={this.state.expanded} />
                ...
                <ScrollView>
                    <View style={{ flex: 1, flexDirection: 'column', marginHorizontal: wp('2%') }}>
                        <View style={{ paddingLeft: wp('2%'), paddingRight: wp('2%'), justifyContent: 'space-between', flexDirection: 'row', marginTop: hp('1%') }}>
                            <Button style={styles.buttonStyle} onPress={() => { this.props.navigation.navigate('AssignmentDetailScreen', { header: this.state.mapping_id ? this.state.mapping_id : 'Scan Each Item' }) }}>
                                            ...
                                            <View style={{
                                                ...
                                                <TouchableOpacity onPress={() => this.checkLevel(item, index)} style={{
                                                    ...
                                                    <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', margin: wp('0.5%'), marginLeft: wp('2%') }}>
                                                        ...
                                                        {item.level_id == 1 ? <Text numberOfLines={1} style={{ flex:1,fontFamily: 'Montserrat-Regular', fontSize: 11, color: '#ffffff', marginLeft: 5,alignSelf: 'center', marginRight:wp('2%') }}>- {item.product_name || '---'}</Text> : null}
                                                    </View>
...
export default withNavigation(AssignSummary);
