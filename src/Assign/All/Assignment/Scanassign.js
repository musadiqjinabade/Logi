...
class Scanassign extends Component {
    ...
    async componentWillMount() {
        this.setState({ model: true, props: this.props.navigation.state.params.item, jobId: this.props.navigation.state.params.item != 'jobHistory' ? this.props.navigation.state.params.item : this.props.navigation.state.params.id }, () => {
    ...
    async componentDidMount() {
        ...
        this.setState({ DashboardRoute: false, model: true, props: this.props.navigation.state.params.item, deviceModel: model, jobId: this.props.navigation.state.params.item != 'jobHistory' ? this.props.navigation.state.params.item : this.props.navigation.state.params.id, seqnumber: {} }, () => {
            this.assignment_mapping()
        });
        this.props.navigation.addListener('didFocus', (item) => {
            BackHandler.addEventListener('hardwareBackPress', this.handleBackPress.bind(this));
            this.setState({ DashboardRoute: false, model: true, props: this.props.navigation.state.params.item, jobId: this.props.navigation.state.params.item != 'jobHistory' ? this.props.navigation.state.params.item : this.props.navigation.state.params.id, seqnumber: {} }, () => {
                this.assignment_mapping()
            });
        });
    }
    ...
    async assignment_mapping() {
        var scan = await APIService.execute('GET', APIService.URLBACKEND + 'assignment/getjobsummary?job_id=' + this.state.jobId, null)
        if (scan.data.data) {
    ...
    render() {
        ...
        return (
            <Container style={{ flex: 1, backgroundColor: '#F1EFEE' }}>
                <Headers isHome={true} onBackHome={() => { this.DashboardRoute() }} label={this.state.jobId ? 'Scan Each Item - ' + this.state.jobId : 'Scan Each Item'} onBack={() => { this.refs.modal6.open() }} expanded={this.state.expanded} />
                                    ...
                                    <QRCodeScanner
                                        ...
                                        cameraStyle={{
                                            height: this.state.model || this.state.delete ? this.state.Updatedata ?
                                                this.state.Updatedata === undefined || this.state.Updatedata == [] ? hgt : hgt - wp('103%') : hgt - wp('18%') : hgt - wp('18%')
                                        }} >
                                    </QRCodeScanner> : null}
                        {this.bottomrender()}
                    </ScrollView>
                    {this.state.Updatedata ?
                        this.state.Updatedata === undefined || this.state.Updatedata.length === 0 ? null :
                            <View style={{
                                ...
                                <TouchableOpacity onPress={() => { this.props.navigation.navigate('AssignSummary', { item: this.state.jobId, url: this.state.seqnumber, jobhistroy: this.props.navigation.state.params.item }) }}>
                                    <LinearGradient
    ...
    onScan(session) {
        ...
        var checkseq = false
        var dataseq = session.newlyRecognizedCodes[0].data;
        var URLVariables = dataseq.split('/')[3]
        ...
        if (checkseq) {
            ...
            if (this.state.delete) {
                ...
                this.setState({ data: session.newlyRecognizedCodes[0].data, delete: true, sURLVariables: sURL_Variables, }, () => {
                    /**** comment*/
                    this.Updatedata();
                    // ToastAndroid.show("Scan Successfully", ToastAndroid.LONG, 25, 50);        
                })
            }
        }
        this.timeout = setTimeout(function () {
    ...
    async getAssignmentsummary(jobId) {
        var scan = await APIService.execute('GET', APIService.URLBACKEND + 'assignment/getjobsummary?job_id=' + jobId, null)
        var responseData = _.map(scan.data.data.assignment_mapping, (item) => {
            return {
                ...item,
                mode: 'Collapsed'
    ...
    async Updatedata() {
        ...
        AsyncStorage.getItem('assignjobId').then(async (value) => {
            if (this.state.jobId != value) {
                var Updatedata = await APIService.execute('POST', APIService.URLBACKEND + 'assignment/getlevelid', data)
                var body = {}
                body.sequence_number = Updatedata.data.data.sequence_number
                var scanstatus = await APIService.execute('POST', APIService.URLBACKEND + 'assignment/checksequenceno', body)
                this.setState({ message: Updatedata.data.message }, () => {
                    if (Updatedata.data.status_code == 200) {
                        if (Updatedata.data.data.level_id == 1) {
                            if (scanstatus.data.assignFlag == false) {
                                ToastAndroid.show(Updatedata.data.message, ToastAndroid.LONG, 25, 50);
                                this.props.navigation.navigate('ProductAssign', { item: Updatedata.data.data, item1: this.state.sURLVariables })
                            }
                            else {
                                ToastAndroid.show(scanstatus.data.message, ToastAndroid.LONG, 25, 50);
                            }

                        } else if (Updatedata.data.data.level_id != null) {
                            this.assignmap(Updatedata.data.data)
                        }
    ...
    async expandReceivedata(seqnum, index) {
        var url = APIService.URLBACKEND + 'assignment/viewjobsummary?job_id=' + this.state.jobId + '&sequence_number=' + seqnum + '&job_type=Assignment_Aggregation';
        ...
        if (this.props.navigation.state.params.item == 'jobHistory') {
            for (let j = 0; j < scan.data.data.length; j++) {
                var seqfull = scan.data.data[j].sequence_number
                // seqnumber.push(scan.data.data.assignment_mapping[i].sequence_number)
                this.setState({ seqnumber: { ...this.state.seqnumber, [seqfull]: !this.state.seqnumber[seqfull] } })
            }
        }
...
export default withNavigation(Scanassign);
