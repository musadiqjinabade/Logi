...
class RMCheckScanningScreen extends Component {
    ...
    async componentDidMount() {
        ...
        this.props.navigation.addListener('didFocus', (item) => {
            ...
            this.setState({ DashboardRoute: false, model: true, jobId: this.props.navigation.state.params.item != 'jobHistory' ? this.props.navigation.state.params.item.job_id : this.props.navigation.state.params.id, seqnumber: {} }, () => {
                this.getJobDetails()
                // this.assignment_mapping()
            });
        });
    ...
    async assignment_mapping() {
        console.log("working", this.state.job_type)
        ...
        if (scan.data.data) {
            ...
            this.setState({ job_detail: scan.data.data.job_detail[0], Updatedata: responseData, job_type: scan.data.data.job_detail[0].job_type }, () => {
                ...
                {this.state.Updatedata ?
                    this.state.Updatedata === undefined || this.state.Updatedata.length === 0 ? null :
                        <View style={{
                            ...
                            <TouchableOpacity onPress={() => { this.props.navigation.navigate('RMCheckScanItemsScreen', { item: this.state.jobId, url: this.state.seqnumber, jobhistroy: this.props.navigation.state.params.item, job_type: this.props.navigation.state.params.item != 'jobHistory' ? this.props.navigation.state.params.item.job_type : this.props.navigation.state.params.type }) }}>
                                <LinearGradient
    ...
    onSuccess = async (e) => {
        ...
        if (e.data.match(new RegExp('/')) !== null) {
            ...
            if (check) {
                ...
                var bodydata = {}
                bodydata.sequence_number = e.data
                var check_seq = await APIService.execute('POST', APIService.URLBACKEND + APIService.RMCheckSequence, bodydata)
                console.log("body for check:", check_seq)
                if (check_seq.data.status == 200) {
                    if (check_seq.data.data.assignFlag == false) {
                        ...
                        if (this.state.delete) {
                            ...
                            this.setState({ data: e, delete: true, sURLVariables: URLVariables, seqnumber: { ...this.state.seqnumber, [URLVariables]: !this.state.seqnumber[URLVariables] } }, () => {
                                setTimeout(() => { this.setState({ timePassed: false }) }, 5000);
                                this.props.navigation.navigate('RMCheckProductAssign', { job_id: this.state.jobId, item1: this.state.sURLVariables })
                            })
                        } else {
                            var sURL_Variables = ''
                            if (e.data.match(new RegExp('/')) !== null) {
                        ...
                        ToastAndroid.show(check_seq.data.data.message, ToastAndroid.LONG, 25, 50);
                    }
                }
                else if (check_seq.data.status == 400) {
                    ToastAndroid.show('Something Went Wrong', ToastAndroid.LONG, 25, 50);
                }
            }
        }
    };
...
export default withNavigation(RMCheckScanningScreen);
