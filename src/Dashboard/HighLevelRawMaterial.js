...
class HighLevelRawMaterial extends Component {
    ...
    async ProductDetails(item) {
        if (this.state.data.data.data.level_id > 1) {
            var data = {}
            data.sequence_number = item.sequence_Id
            ...
            if (Updatedata.data.data.level_id > 2) {
                ...
                console.log("data for shipper")
                this.props.navigation.navigate("ShipperLevelRawMaterial", { item: Updatedata });
            }
        }
        else if (this.state.data.data.data.level_id == 1) {
            var body_data = {}
            body_data.sequence_number = item.sequence_Id

            var ResponseData = await APIService.execute('POST', APIService.URLBACKEND + 'scanCheck/rawmaterial/scanandcheck', body_data)
            if (ResponseData.data.status_code == 200) {
                this.props.navigation.navigate("SKUlevelRawMaterial", { item: ResponseData });
            }
    ...
    render() {
        return (
            <Container style={{ flex: 1, backgroundColor: '#F1F3FD' }}>
                <Headers isHome={true} onBackHome={() => { this.props.navigation.navigate('Dashboard') }} label={this.state.data ? this.state.data.data.data.level_name : '---'} onBack={() => { this.props.navigation.goBack() }} expanded={this.state.expanded} />
                ...
                <View style={styles.linearGradient}>
                    <View style={{ flexDirection: 'row', backgroundColor: '#F1F3FD', justifyContent: 'space-between', alignItems: 'center', margin: widthPercentageToDP('2%') }}>
                        <TouchableOpacity
                            ...
                            <Text style={{ justifyContent: 'center', alignItems: 'center', fontFamily: 'Montserrat-Bold', color: '#1D3567' }}>{this.state.data ? this.state.data.data.data.level_name : '---'}</Text>
                            ...
                            this.state.scanhistory ?
                                <View style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', alignItems: 'center', margin: widthPercentageToDP('2%') }}>
                                    <View
                                        ...
                                        <Text onContentSizeChange={(e) => this.updateSizeProduct(e.nativeEvent.contentSize.height)} style={styles.selectproducts}>{this.state.data ? this.state.data.data.data.level_name : 'pallet'}: {this.state.data ? this.state.data.data.data.sequence_Id : '--'}</Text>
                                    </View>
                                    <View
                                        onContentSizeChange={(e) => this.updateSizeProduct(e.nativeEvent.contentSize.height)}
                                        style={{
                                        ...
                                        <SvgUri width="22" height="12" svgXmlData={svgImages.checkboxgreen} style={{ resizeMode: 'contain', justifyContent: 'flex-start', alignItems: 'center' }} />
                                        <Text onContentSizeChange={(e) => this.updateSizeProduct(e.nativeEvent.contentSize.height)} style={styles.selectproducts}>Truck UID: {this.state.data && this.state.data.data.data.Parent_sequence_Id ? this.state.data.data.data.Parent_sequence_Id : '--'}</Text>
                                        {this.state.data && this.state.data.data.data.Parent_sequence_Id != null ?
                                            <TouchableOpacity onPress={() => this.props.navigation.navigate('ScanMoveRawMaterial', { item: this.state.data.data.data.Parent_sequence_Id })} style={{ flex: 1, flexDirection: 'row', justifyContent: 'flex-end' }}>
                                    ...
                                    <View
                                        ...
                                        <SvgUri width="22" height="12" svgXmlData={svgImages.checkboxgreen} style={{ resizeMode: 'contain', justifyContent: 'flex-start', alignItems: 'center' }} />
                                        <Text onContentSizeChange={(e) => this.updateSizeProduct(e.nativeEvent.contentSize.height)} style={{ flex: 1 }}>Product name: {this.state.data && this.state.data.data.data ? this.state.data.data.data.product_name : '----'}</Text>
                                    </View>
                                    <View
                                        onContentSizeChange={(e) => this.updateSizeProduct(e.nativeEvent.contentSize.height)}
                                        style={{
                                    ...
                                    <View style={{
                                        ...
                                        <View style={{
                                            ...
                                            <FlatList
                                                extraData={this.state.data ? this.state.data.data.data.otherDetails : ''}
                                                data={this.state.data ? this.state.data.data.data.otherDetails : ''}
                                                renderItem={({ item, index }) => (

                                                    <TouchableOpacity
                                                        onPress={() => { this.ProductDetails(item) }} style={{ flexDirection: 'row', flex: 1, justifyContent: 'space-between', alignItems: 'flex-start', marginHorizontal: widthPercentageToDP('2%') }}>
                                                        <View style={{
                                                            flexDirection: 'row', width: widthPercentageToDP('39%'), height: widthPercentageToDP('6%'),
                                                            backgroundColor: '#FFFFFF', justifyContent: 'flex-start', alignItems: 'center',
                                                        }}>
                                                            <Text style={{ fontFamily: 'Montserrat-Regular', color: '#1D3567' }}>{item.sequence_Id || '--'}</Text>
                                                        </View>
                                                        {/* <View style={{justifyContent:'flex-end', alignItems:'flex-end',margin:widthPercentageToDP('2%'), flex:1, flexDirection:'row',padding:widthPercentageToDP('1%'),borderWidth:1}}> */}
                                                        <View style={{
                                                        ...
                                                        <View style={{
                                                            ...
                                                            <Text style={{ flex: 1, margin: 2, color: '#1D3567', fontFamily: 'Montserrat-Regular', justifyContent: 'center', alignItems: 'center' }}>{item.product_count}</Text>
                                                        </View>
                                                        {/* </View> */}
                                                    </TouchableOpacity>
                                                )} />
...
export default withNavigation(HighLevelRawMaterial);
