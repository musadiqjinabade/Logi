...
const resetAction = StackActions.reset({
    index: 0,
    actions: [NavigationActions.navigate({ routeName: 'RecieveScanSummary' })],
});
class ReceiveStockDetailsScreen extends Component {
    ...
    async cancelJob() {
        ...
        var data = {}
        data.job_type = "Recieve"
        data.job_id = this.state.JobId ?  this.state.JobId : null
    ...
    render() {
        return (
            <SafeAreaView style={{ flex: 1, flexDirection: 'column' }}>
                <Headers isHome={true} onBackHome={() => { this.refs.redirectHome.open()  }}  label={'Receive Stock - ' + this.state.JobId}  onBack={() => this.props.navigation.goBack() }expanded={this.state.expanded} />
...
export default withNavigation(ReceiveStockDetailsScreen);
