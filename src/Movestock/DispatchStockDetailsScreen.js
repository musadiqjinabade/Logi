...
import { Text, View, StatusBar, SafeAreaView, FlatList, StyleSheet, TouchableOpacity,ToastAndroid, AsyncStorage, BackHandler } from 'react-native';
...
const resetAction = StackActions.reset({
    index: 0,
    actions: [NavigationActions.navigate({ routeName: 'MoveScanSummary' })],
});
class DispatchStockDetailsScreen extends Component {
    ...
    render() {
        return (
            <SafeAreaView style={{ flex: 1, flexDirection: 'column' }}>
                <Headers isHome={true} onBackHome={() => { this.refs.redirectHome.open() }}  label={'Dispatch Stock - ' + this.state.jobId} onBack={() => this.props.navigation.goBack()} expanded={this.state.expanded} />
...
export default withNavigation(DispatchStockDetailsScreen);
