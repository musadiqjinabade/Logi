...
class ProductReturnCheckValidation extends Component {
    ...
    componentDidMount() {
        this.setState({ job_id: this.props.navigation.state.params.data, job_detail: this.props.navigation.state.params.job_detail }, () => {
            this.getCheckValidation()
            console.log("this.state.job_detail", this.state.job_detail)
        })
    }
        ...
        if (this.state.checkvalidation) {
            return (
                <Container style={{ flex: 1, backgroundColor: '#F1EFEE' }}>
                    <Headers label={this.state.job_id ? "Product Return - " + this.state.job_id : "Product Return"} onBack={() => this.props.navigation.goBack()} expanded={this.state.expanded} />
                        ...
                        <ScrollView>
                            <View style={{ flex: 1, marginTop: widthPercentageToDP('4%') }}>
                                ...
                                <View style={{ marginTop: heightPercentageToDP('1%'), flex: 1, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    ...
                                    <Text style={{ width: widthPercentageToDP('30%'), fontFamily: 'Montserrat-Regular', color: '#1D3567', justifyContent: 'flex-end', alignSelf: 'center' }}>{this.state.job_detail ? this.state.job_detail.document_type : '-'}</Text>
                                </View>
                                <View style={{ marginTop: heightPercentageToDP('1%'), flex: 1, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    ...
                                    <Text style={{ width: widthPercentageToDP('30%'), fontFamily: 'Montserrat-Regular', color: '#1D3567', justifyContent: 'flex-end', alignSelf: 'center' }}>{this.state.job_detail ? this.state.job_detail.document_no : '-'}</Text>
                                </View>
            ...
            return (
                <Container style={{ flex: 1, backgroundColor: '#F1EFEE' }}>
                    <Headers label={"Product Return"} onBack={() => { this.props.navigation.goBack() }} expanded={this.state.expanded} />
                </Container>
            )
...
export default withNavigation(ProductReturnCheckValidation);
