...
const resetAction = StackActions.reset({
    index: 0,
    actions: [NavigationActions.navigate({ routeName: 'ProductReturnScanningItemsScreen' })],
});
const resetAction1 = StackActions.reset({
    index: 0,
    actions: [NavigationActions.navigate({ routeName: 'Dashboard' })],
});
class ProductReturnScanningDetailScreen extends Component {
    ...
    async componentDidMount() {
        ...
        this.focusListener = this.props.navigation.addListener('didFocus', async () => {
            ...
            var resArray = []
            if (reasons != null) {
                resArray = JSON.parse(reasons)
            }
            this.setState({ reasonsData: resArray })
            var Selectedreasons = await AsyncStorage.getItem('Selectedreasons')
            var SelectedArrayReasons = []
            if (Selectedreasons != null) {
                SelectedArrayReasons = JSON.parse(Selectedreasons)
            }


            this.setState({ SelectedTags: SelectedArray, Selectedreasons: SelectedArrayReasons })
            // this.getDetails()
    ...
    async getReturnReasons() {
        this.props.navigation.navigate('SearchableReturnReasons', { item: this.state.reasonsData })
    }
            ...
            <SafeAreaView style={{ flex: 1, flexDirection: 'column' }}>
                ...
                <ScrollView style={{ marginBottom: hp('8%') }}>
                    <View style={{ flex: 1, flexDirection: 'column' }}>
                        ...
                        <TouchableOpacity onPress={() => this.getReturnReasons()} style={{ marginLeft: wp('2%'), marginRight: wp('2%'), flexDirection: 'column' }}>
                            <View style={{ marginTop: hp('2%'), height: wp('6%'), flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', marginBottom: wp('2%') }}>
                                <Text style={styles.header}>Return Reasons</Text>
                            </View>
                            {this.state.reasonsData !== null && this.state.reasonsData.length > 0 ?
                                <View style={styles.selectBackgroundStyle} >
                                    <Text style={styles.selectValues}>{' (' + this.state.reasonsData.length + ') Tags selected'}</Text>
                                ...
                                <View style={styles.selectBackgroundStyle} >
                                    <Text style={styles.selectValues}>{"Select Reasons"}</Text>
                                    <SvgUri width="17" height="13" svgXmlData={svgImages.drop_down_arrow} style={{ resizeMode: 'contain', justifyContent: 'flex-end', alignItems: 'center', marginTop: wp('4%'), marginRight: wp('4%') }} />
                                </View>
                            }
                        </TouchableOpacity>

                        {this.state.Selectedreasons !== null && this.state.Selectedreasons.length > 0 ? <View style={{
                            ...
                            <FlatList
                                ...
                                numColumns={2}
                                data={this.state.Selectedreasons}
                                renderItem={({ item }) => {
...
export default withNavigation(ProductReturnScanningDetailScreen);
